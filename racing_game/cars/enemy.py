...
class EnemyPlayer(Car):
    ...
    def car_current_speed(self):
        ...
        if self.car_speed < 0:
            DrawUI.draw_text(f"0", normal_font, "red", position_x, position_y, GAME_SCREEN)
        if self.car_speed == 0:
            DrawUI.draw_text(f"{round(self.car_speed)}", normal_font, "white", position_x, position_y, GAME_SCREEN)
        if 0 < self.car_speed <= 1:
            DrawUI.draw_text(f"{round(self.car_speed)}", normal_font, "green", position_x, position_y, GAME_SCREEN)
        if 1 < self.car_speed <= 2:
            DrawUI.draw_text(f"{round(self.car_speed)}", normal_font, "green", position_x, position_y, GAME_SCREEN)
        if 2 < self.car_speed <= 3:
            DrawUI.draw_text(f"{round(self.car_speed)}", normal_font, "orange", position_x, position_y, GAME_SCREEN)
        if 3 < self.car_speed <= 20:
            DrawUI.draw_text(f"{round(self.car_speed)}", normal_font, "red", position_x, position_y, GAME_SCREEN)

    def car_current_nitro(self):
        DrawUI.draw_text(f"{round(self.car_nitro)}", normal_font, "cyan", 200, 740, GAME_SCREEN)
